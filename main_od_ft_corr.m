...
% This reconstruction converts it to  inverse problem formulation using
% autocorrelation of fourier transform of reflectivity function

% Pragya Sharma, Cornell University, Dec. 09, 2016
function [] = main_od_ft_corr
clear all;
...
b = 1;
m = 1;
k1 = 0.2; % Choosing shifting factor of scatterer position in x and y direction
k2 = -0.15;
RCS = 1;
...
switch(f)        
    case 'f2'
        Freq = [2200000000.00000,2100000000.00000,2000000000.00000];        
    case 'f3'
    ...
    case 'f4'
        ...
        Freq = 2*[0.721 0.750 0.829 0.889 0.950 0.996 1.045 1.060]*1e9;
...
Pos_tag = l* Pos_tag;
Pos_tag_mean=Pos_tag; %+2*(rand(Ntag,2)-1)*0.007
...
G = reshape(G_calib,[],1); % all tag, recv, freq
assignin('base','G',G)
N = length(G);
V = G;
assignin('base','V',V);
...
- repmat(permute(Pos_tag_mean, [3 1 2]), [lx*lx 1 1]) ).^2, 3)),1,Nrecv);
...
% B = diag(e'*(V*V')*e);
B = sum((e'*R).*transpose(e),2); % faster doens't calc off-diagonal elements
assignin('base','B',B);
B = real(B);
B = (reshape(B, [lx lx]))';
